...
function getCountP(uint _candidateId) timer public returns (uint) { 
    return person[_candidateId].votecount;
}

function getCountL(uint _candidateId) timer public returns (uint) {
    return list[_candidateId].votecount;
}
 //final result
function resultP() public returns(uint) {
    uint max=0;
...
function resultL() public returns(uint) {
    uint max=0;
